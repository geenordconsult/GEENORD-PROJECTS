...
INPUT_RANGES = {
    'N': {'min': 0.0, 'max': 145.0, 'default': 90.0, 'help': 'Available Nitrogen in soil (e.g., mg/kg or ppm).'},
    'P': {'min': 5.0, 'max': 150.0, 'default': 42.0, 'help': 'Available Phosphorus in soil (e.g., mg/kg or ppm).'},
    'K': {'min': 5.0, 'max': 210.0, 'default': 43.0, 'help': 'Available Potassium in soil (e.g., mg/kg or ppm).'},
    'ph': {'min': 3.5, 'max': 10.5, 'default': 5.5, 'help': 'Soil acidity/alkalinity level (3.5 = highly acidic, 9.5 = highly alkaline).'}
}
...
st.sidebar.info(
   "This application uses a trained **Machine Learning Classification Model** "
    ...
    "\n\n---"
    "\n **Range of Values for Soil Nutrient Parameters**"
    "\n\n\t  NITROGEN: 0.00 - 140.00"
    "\n\n\t  POTASSIUM: 5.00 - 205.00"
    "\n\n\t  PHOSPHOROUS: 5.00 - 145.00"
    "\n\n\t  PH Value: 3.5.00 - 10.00"
...
if submitted:
    ...
    if ph < 3.5 or ph > 10.0:
        st.warning("🚨 Warning: pH value is outside the optimal range (3.5 - 10.0) for the crops. Prediction may be inaccurate.")
        is_valid = False

    if N > 140 :
        st.warning("🚨 Warning: Nitrogen value is outside the optimal range (0 - 140.0) for the crops. Prediction may be inaccurate.")
        is_valid = False
    # Proceed only if the core validation passes (we continue with prediction for warnings)

    if K < 5 or K > 205 :
        st.warning("🚨 Warning: Potassium value is outside the optimal range (5.0 - 205.0) for the crops. Prediction may be inaccurate.")
        is_valid = False
    # Proceed only if the core validation passes (we continue with prediction for warnings)
    if P < 5 or P > 145:
        st.warning("🚨 Warning: Phosphorous value is outside the optimal range (5.0 - 145.0) for the crops. Prediction may be inaccurate.")
        is_valid = False
    # Proceed only if the core validation passes (we continue with prediction for warnings)
